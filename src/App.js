...
const renewMultipleContracts = (contractIdsFilter = null) => {
  ...
  // Si rien n'a été entré (ni % ni montant fixe, et pas de prix par unité), on bascule
  // automatiquement en mode "un par un" sans reposer la question, comme avant.
  const rienEntreParDefaut = !useUnitPricing && !defaultMajType;

  const reviewIndividually = rienEntreParDefaut
    ? true
    : window.confirm(
...
const paymentAmount = contract.amount / numPayments;
    ...
    const contractHTML = `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 22px; line-height: 1.55; font-size: 12px; page-break-after: always;">
        <div style="background: linear-gradient(135deg, #1a4d1a 0%, #2d5a27 100%); color: white; padding: 16px 12px; border-radius: 8px; text-align: center; margin-bottom: 15px;">
          ...
          <p style="color: white; margin-bottom: 4px; font-size: 10px;"><strong>Maxim Pominville</strong></p>
          <p style="color: white; margin-bottom: 8px; font-size: 10px;"><strong>Téléphone: 514-444-6324</strong></p>
  ...
  <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Informations du Client :</h3>
  <p style="margin: 7px 0;"><strong>• Prénom et Nom :</strong> ${client.name}</p>
  <p style="margin: 7px 0;"><strong>• Adresse du Service :</strong> ${client.address}</p>
  <p style="margin: 7px 0;"><strong>• Numéro de Téléphone :</strong> ${client.phone}</p>
  ${client.phone2 ? `<p style="margin: 7px 0;"><strong>• Téléphone 2 :</strong> ${client.phone2}</p>` : ''}
  ${client.email ? `<p style="margin: 7px 0;"><strong>• Courriel :</strong> ${client.email}</p>` : ''}
</div>        
${(contract.entreesCompletes || contract.devantsTempo || contract.stationnementsCommerciaux) ? `
<hr style="border: 1px solid #a8c5a8; margin: 11px 0;">
<div style="margin-bottom: 14px;">
  <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Détails du Service :</h3>
  ${contract.entreesCompletes ? `<p style="margin: 7px 0;"><strong>• Entrée(s) complète(s) :</strong> ${contract.entreesCompletes}</p>` : ''}
  ${contract.instructionsEntrees ? `<p style="margin: 6px 0; margin-left: 20px; font-style: italic; color: #495057;">↳ ${contract.instructionsEntrees}</p>` : ''}
  ${contract.devantsTempo ? `<p style="margin: 7px 0;"><strong>• Devant(s) seulement (tempo) :</strong> ${contract.devantsTempo}</p>` : ''}
  ${contract.instructionsTempo ? `<p style="margin: 6px 0; margin-left: 20px; font-style: italic; color: #495057;">↳ ${contract.instructionsTempo}</p>` : ''}
  ${contract.stationnementsCommerciaux ? `<p style="margin: 7px 0;"><strong>• Stationnement(s) commercial / multi-logements :</strong> ${contract.stationnementsCommerciaux}</p>` : ''}
  ${contract.instructionsCommercial ? `<p style="margin: 6px 0; margin-left: 20px; font-style: italic; color: #495057;">↳ ${contract.instructionsCommercial}</p>` : ''}
</div>
` : ''}
        <hr style="border: 1px solid #a8c5a8; margin: 11px 0;">
        
        <div style="margin-bottom: 14px;">
          <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Conditions Générales du Service :</h3>
          ...
          <p style="margin: 7px 0;"><strong>1. Déclenchement du Service :</strong> Le service de déneigement débutera lorsque l'accumulation de neige atteindra un minimum de cinq (5) centimètres.</p>
          <p style="margin: 7px 0;"><strong>2. Protection de la Propriété :</strong> Il est de la responsabilité exclusive du client de protéger adéquatement ses arbres, arbustes, ainsi que tout autre objet décoratif ou aménagement paysager situé dans la zone de déneigement.</p>
          <p style="margin: 7px 0;"><strong>3. Libération des Aires de Stationnement :</strong> Le client s'engage à libérer les aires de stationnement et d'accès de tout objet mobile avant chaque intervention de déneigement.</p>
          <p style="margin: 7px 0;"><strong>4. Déplacement des Véhicules :</strong> Les véhicules doivent être retirés des entrées de stationnement avant 9h00 du matin le lendemain de la tempête pour permettre le déneigement complet. <strong style="color: #d32f2f;">Des frais supplémentaires de 30$ seront facturés si nous devons ressortir en raison de véhicules non déplacés ou d'obstacles non prévus.</strong></p>
          <p style="margin: 7px 0;"><strong>5. Installation des Piquets :</strong> Les piquets de délimitation seront installés uniquement après réception du paiement intégral ou du premier versement du contrat.</p>
        </div>
        
        ${contract.notes ? `
        <hr style="border: 1px solid #a8c5a8; margin: 11px 0;">
        <div style="margin-bottom: 14px;">
          <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Notes Spéciales :</h3>
        ...
        <div style="margin-bottom: 14px;">
          <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Période de Validité :</h3>
          <p style="margin: 7px 0;">Le présent contrat entre en vigueur le <strong>${contract.startDate}</strong> et demeure valide jusqu'au <strong>${contract.endDate || '31 mars ' + (new Date().getFullYear() + 1)}</strong>.</p>
        </div>

        <hr style="border: 1px solid #a8c5a8; margin: 11px 0;">

        <div style="margin-bottom: 14px;">
          <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Modalités de Tarification et de Paiement :</h3>
          <p style="margin: 7px 0;">Le tarif pour la saison de déneigement est fixe et établi comme suit :</p>
          <p style="margin: 4px 0; font-size: 12px;"><strong>• Montant Total du Contrat :</strong> ${contract.amount.toFixed(2)} $</p>
          
${paymentStructure === '1' ? `
  <p style="margin: 7px 0;">Le paiement s'effectuera en un versement unique :</p>
  <div style="margin: 7px 0;">
    <p style="margin: 7px 0;"><strong>• Paiement Unique :</strong></p>
    <p style="margin: 6px 0; margin-left: 20px;">• Date : ${client.firstPaymentDate || contract.startDate}</p>
    <p style="margin: 6px 0; margin-left: 20px;">• Montant : ${paymentAmount.toFixed(2)} $</p>
  </div>
` : `
  <p style="margin: 7px 0;">Le paiement s'effectuera en ${paymentStructure} versements :</p>
  
  <!-- 1er versement -->
  <div style="margin: 7px 0;">
    <p style="margin: 7px 0;"><strong>• 1er Versement :</strong></p>
    <p style="margin: 6px 0; margin-left: 20px;">• Date : ${client.firstPaymentDate || contract.startDate}</p>
    <p style="margin: 6px 0; margin-left: 20px;">• Montant : ${paymentAmount.toFixed(2)} $</p>
  </div>
  ...
  <!-- 2e versement -->
  <div style="margin: 7px 0;">
    <p style="margin: 7px 0;"><strong>• 2e Versement :</strong></p>
    <p style="margin: 6px 0; margin-left: 20px;">• Date : ${client.secondPaymentDate || 'À déterminer'}</p>
    <p style="margin: 6px 0; margin-left: 20px;">• Montant : ${paymentAmount.toFixed(2)} $</p>
  </div>
  ...
  ${(paymentStructure === '3' || paymentStructure === '4') ? `
    <!-- 3e versement -->
    <div style="margin: 7px 0;">
      <p style="margin: 7px 0;"><strong>• 3e Versement :</strong></p>
      <p style="margin: 6px 0; margin-left: 20px;">• Date : ${client.thirdPaymentDate || 'À déterminer'}</p>
      <p style="margin: 6px 0; margin-left: 20px;">• Montant : ${paymentAmount.toFixed(2)} $</p>
    </div>
  ` : ''}
  ...
  ${paymentStructure === '4' ? `
    <!-- 4e versement -->
    <div style="margin: 7px 0;">
      <p style="margin: 7px 0;"><strong>• 4e Versement :</strong></p>
      <p style="margin: 6px 0; margin-left: 20px;">• Date : ${client.fourthPaymentDate || 'À déterminer'}</p>
      <p style="margin: 6px 0; margin-left: 20px;">• Montant : ${paymentAmount.toFixed(2)} $</p>
    </div>
  ` : ''}
`}
        ...
        <div style="margin-bottom: 14px;">
          <h3 style="color: #1a4d1a; font-size: 12px; margin-bottom: 8px; font-weight: bold; border-bottom: 2px solid #1a4d1a; padding-bottom: 5px;">Acceptation du Contrat :</h3>
          <p style="margin: 7px 0;">En signant ci-dessous, le client et le représentant de JM Pominville reconnaissent avoir lu, compris et accepté l'intégralité des clauses et conditions énoncées dans le présent contrat.</p>
        </div>
  ...
  newWindow.document.write(`
    <html>
      ...
      <body>
        <div class="close-window-bar" style="position: sticky; top: 0; z-index: 999; background: #1a4d1a; padding: 12px; text-align: center;">
          <button onclick="window.close()" style="padding: 10px 24px; background: white; color: #1a4d1a; border: none; border-radius: 8px; font-weight: bold; font-size: 15px; cursor: pointer;">
...
` : `
  ...
  ` : ''}
`}          
          <p style="margin: 4px 0; font-size: 11px;">Tout retard de paiement pourrait entraîner la suspension immédiate du service de déneigement jusqu'à la régularisation du solde.</p>
        ...
        <div style="margin-top: 4px; display: table; width: 100%; table-layout: fixed;">
          ...
          <div style="display: table-cell; width: 48%; vertical-align: top;">
            ...
            <div style="margin: 4px 0; height: 45px; width: 150px; overflow: hidden; display: flex; align-items: center; justify-content: flex-start;">
<img src="data:image/png;base64,"

           alt="Signature Maxim Pominville" 
                   style="display: block; max-height: 42px; max-width: 150px; width: auto; height: auto;" />
